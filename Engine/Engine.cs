...
static partial class Engine
{
    ...
    public static Texture RenderTarget;
    ...
    private static void Start()
    {
        ...
        // ARYAN: allow for functionality on non-windows platforms
        ...
        SDL_mixer.MIX_InitFlags mixInitFlags = SDL_mixer.MIX_InitFlags.MIX_INIT_MP3 |
        ...
        IntPtr renderTargetHandle = SDL.SDL_CreateTexture(Renderer, SDL.SDL_PIXELFORMAT_RGBA8888,
            (int)SDL.SDL_TextureAccess.SDL_TEXTUREACCESS_TARGET, (int)Game.Resolution.X, (int)Game.Resolution.Y);
    ...
    {
        var lastFrameStartTime = SDL.SDL_GetPerformanceCounter();
        ...
        while (true)
        {
            ...
            if (TimeDelta > (1/30.0f)) TimeDelta = (1 / 30.0f); // Prevents the game from slowing down past 30fps
            ...
            if (GetKeyDown(Key.Return) && (GetKeyHeld(Key.LeftAlt) || GetKeyHeld(Key.RightAlt)))
            {
                ...
                SDL.SDL_SetWindowFullscreen(Window,
                    Fullscreen ? (uint)SDL.SDL_WindowFlags.SDL_WINDOW_FULLSCREEN_DESKTOP : 0);
            }
    ...
    private static void MirrorDirectory(string sourceDirectory, string targetDirectory, bool deleteUnusedFiles)
    {
        ...
        if (deleteUnusedFiles)
        {
            ...
            foreach (string targetDir in Directory.GetDirectories(targetDirectory, "*", SearchOption.AllDirectories))
            {
