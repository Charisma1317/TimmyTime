...
static partial class Engine
{
    ...
    public static HashSet<MouseButton> MouseButtonsHeld = new HashSet<MouseButton>();
    ...
    public static HashSet<Key> KeysHeld = new HashSet<Key>();
    ...
    private static readonly Dictionary<int, Action<KeyEvent>> KeyListeners = new Dictionary<int, Action<KeyEvent>>();
    private static int _keyListenerIdCounter = 0;
    ...
    public static int AddKeyListener(Action<KeyEvent> callback)
    {
        var id = _keyListenerIdCounter++;
        KeyListeners.Add(id, callback);
    ...
    private static void _broadcastKeyDown(Key e)
    {
        KeyListeners.Values.ToList().ForEach(callback => callback(new KeyEvent { Type = KeyEventType.Down, Key = e }));
    }

    private static void _broadcastKeyUp(Key e)
    {
        KeyListeners.Values.ToList().ForEach(callback => callback(new KeyEvent { Type = KeyEventType.Up, Key = e }));
    }

    private static void _broadcastKeyHeld(Key e)
    {
        KeyListeners.Values.ToList().ForEach(callback => callback(new KeyEvent { Type = KeyEventType.Held, Key = e }));
    }

    private static void _broadcastTextTyped(char e)
    {
        KeyListeners.Values.ToList().ForEach(callback => callback(new KeyEvent { Type = KeyEventType.Text, Text = e}));
    }

    private static void PollEvents()
    {
        // ======================================================================================
        // Broadcast any frame-independent input events
        // ======================================================================================

        foreach (var k in KeysHeld)
        {
            _broadcastKeyHeld(k);
        }
        ...
        while (SDL.SDL_PollEvent(out evt) != 0)
        {
            ...
            else if (evt.type == SDL.SDL_EventType.SDL_KEYDOWN)
            {
                if (evt.key.repeat == 0)
                {
                    _broadcastKeyDown((Key)evt.key.keysym.sym);
                    _broadcastKeyHeld((Key)evt.key.keysym.sym);
            ...
            {
                _broadcastKeyUp((Key)evt.key.keysym.sym);
...
public struct KeyEvent
{
    ...
    {
        return $"KeyEvent {{ type={Enum.GetName(typeof(KeyEventType), Type)}, key={Key}, text={Text} }}";
    }
}
